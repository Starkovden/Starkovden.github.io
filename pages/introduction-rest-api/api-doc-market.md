...
- [Нативные библиотеки API (например API для Java, C++ и .NET)](Overview-of-library.html);
- [REST API](what-is-rest-api.html).
...
{% include image.html file="introduction-rest-api/19.png" alt="firstFactor" caption="Опрос показал, что полная и точная документация играет огромную роль для разработчиков" %}
...
{% include image.html file="introduction-rest-api/21.png" alt="apigrowth" caption="Феноменальный рост API" %}
...
{% include image.html file="introduction-rest-api/22.png" alt="externalAPI" caption="Многие сайты используют необходимые сервисы через внешние API" %}
...
{% include image.html file="introduction-rest-api/23.png" alt="resume" caption="Описание вакансии, где ищут человека, умеющего создавать статический сайт в стиле Dropbox" %}
...
{% include image.html file="introduction-rest-api/24.png" alt="dropboxAPI" caption="Сайт API разработчика имеет простой и ясный интерфейс" %}

Здесь нет сложного дизайна. Но простота и краткость делают его привлекательным. Если учесть, что документация по API в большей или меньшей степени является интерфейсом продукта, создание четкого, современно оформленного сайта документации имеет первостепенное значение для доверия и популярности на рынке. ([В рынок труда погрузимся позже](job-market.html))
...
{% include image.html file="introduction-rest-api/25.png" alt="intercom" caption="Статья Intercom, сфокусированная на проблеме API документации" %}
