...
После изучения модуля ["Используем API как разработчики"](about-second-module.html), откроются новые горизонты и простой технический писатель станет техническим писателем, которому может документировать конечные точки API.
