...
Нет необходимости изучать модули по порядку - можно "гулять" по ним по своему усмотрению. Но некоторые из модулей (например, [Используем REST API в роли разработчика](about-second-module.html) и [Конечные точки API](about-third-module.html)) все-таки должны быть последовательны.
...
Видео записи курса [здесь](video-presentations.html). Самым последним полнометражным видео всего курса является семинар по API, который я провел в Менло-Парке, штат Калифорния, в ноябре 2018 года. Этот видеофрагмент не настолько детализирован, как в письменном материале, но будет быть хорошим началом.  
